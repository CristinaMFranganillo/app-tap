...
export class FormEscuadraEntrenamientoComponent {
  ...
  get tarifaSocio(): number { return this.tarifas().find(t => t.tipo === 'socio')?.importe ?? 6; }
  get tarifaNoSocio(): number { return this.tarifas().find(t => t.tipo === 'no_socio')?.importe ?? 7; }
  ...
  get countSocios(): number {
    let c = 0;
    for (let i = 0; i < 6; i++) {
      if (this.tipos[i] === 'socio' && this.userIds[i]) c++;
    }
    return c;
  }
